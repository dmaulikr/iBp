...
+ (NSArray *)countryInfos {
    ...
    return [userCountry objectForKey:@"userCountry"];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_PCCs) {
        ...
        NSString *strCountryCode = [[NSString stringWithFormat:@"%@",countryDic[@"country_code"]]lowercaseString];
        NSURL *url = [[NSBundle mainBundle] URLForResource:strCountryCode withExtension:@"png"];
        
        NSData * imageData = [[NSData alloc] initWithContentsOfURL: url];
        cell.imageView.image = [UIImage imageWithData: imageData];
        
       /* [cell.imageView startLoaderWithTintColor:[UIColor greenColor]];
        [cell.imageView sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:@"flag_missing.png"] options:SDWebImageCacheMemoryOnly | SDWebImageRefreshCached progress:^(NSInteger receivedSize, NSInteger expectedSize) {
            [cell.imageView updateImageDownloadProgress:(CGFloat)receivedSize/expectedSize];
        } completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            [cell.imageView reveal];
        }];*/
...
+ (id)infoForPhoneCode:(NSInteger)phoneCode{
    ...
    id result = nil;
    for (NSDictionary * dic in array) {
        if (phoneCode == [dic[@"dialing"] integerValue]) {
            result = dic;
            break;
        }
    }
