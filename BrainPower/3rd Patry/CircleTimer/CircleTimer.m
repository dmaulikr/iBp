...
#define FONT UIFontAvenirNextBold(15)
...
- (void)baseInit {
    ...
    self.font = UIFontAvenirNextBold(15);
    /*if (iPhoneVersion == 4 || iPhoneVersion == 5)
        self.font = UIFontAvenirNextBold(15);
    else if (iPhoneVersion == 6 || iPhoneVersion == 61)
        self.font = UIFontAvenirNextBold(20);
    else
        self.font = UIFontAvenirNextBold(30);*/
...
- (void)setActive:(BOOL)active {
    _active = active;
    if (active) {
        [self updateTimerLabel: self.elapsedTime:_totalTime];
        self.timerLabel.textColor = self.fontColor;
    } else {
        [self updateTimerLabel:self.totalTime:_totalTime];
...
- (void)setElapsedTime:(NSTimeInterval)elapsedTime {
    if (_elapsedTime != elapsedTime) {
        ...
        [self updateTimerLabel:elapsedTime:_totalTime];
    }
}

- (void)updateTimerLabel:(NSTimeInterval)elapsedTime :(NSTimeInterval)totaltime
{
    int hour    = (int) ((totaltime - elapsedTime) /3600);
    int minutes = (int) ((totaltime - elapsedTime) / 60)%60;
    
    int seconds = (int) (totaltime - elapsedTime) % 60;
